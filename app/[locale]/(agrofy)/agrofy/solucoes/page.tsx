 ...
 * `(mineracao)/solucoes/page.tsx` (same layout: hero + alternating
 * image/text product blocks + CTA). Every product from the Agrofy catalog
 * lives here now; the `/agrofy` home only teases them via
 * `AgroBentoSolutions` linking to `#<id>` anchors on this page.
 */
...
export default async function AgrofySolutionsPage({ params }: Props) {
  ...
  const t = await getTranslations("agrofySolutions");
    ...
    <div className="agro-theme min-h-screen bg-background">
      ...
      <section className="relative flex min-h-[60vh] items-center overflow-hidden pt-24">
        <Image
          ...
          alt="Interior da cabine de um trator com tablet embarcado"
          fill
          priority
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      {item.tiers.map((tier) => (
                        <div
                          key={tier.name}
                          className="group flex flex-col rounded-2xl border border-border bg-card overflow-hidden transition-all duration-300 hover:border-[#16A34A]/50 hover:-translate-y-1"
                        >
