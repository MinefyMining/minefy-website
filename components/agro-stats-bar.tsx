...
function useCountUp(target: string, isVisible: boolean) {
  ...
  // scrolls into view on a client that allows motion. Keeps the initial HTML
  // honest: crawlers/readers/no-JS visitors never see "0".
...
function StatCounter({ value, label }: StatItem) {
  ...
  return (
    <div ref={ref} className="flex flex-col items-center justify-center gap-1 py-6 px-4">
      <span className="text-2xl md:text-3xl font-bold text-[#16A34A] tabular-nums leading-none">
        {display}
      </span>
...
/**
 * Green mirror of `StatsBar`, dedicated to the Agrofy home. Reads from the
 * `agrofy.stats.items` namespace instead of `home.stats.items` and swaps the
 * `text-primary` accent (a global CSS var, gold, not scoped by `.agro-theme`)
 * for a hardcoded green — same reasoning documented across the other
 * `agro-*` clones.
 *
 * Unlike the mining stats bar (real platform/adoption numbers — 280+
 * tablets, #1 mineradora), Agrofy has no client base yet, so these are
 * sourced market facts (PIB do agro, participação no PIB, alta no roubo de
 * máquinas) — never invented traction. See the disclaimer rendered by the
 * caller under this component.
 */
export function AgroStatsBar() {
  const t = useTranslations("agrofy");
